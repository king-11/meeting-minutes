...
class CustomAIService:
    ...
    def __init__(self, api_url: str = None, api_key: Optional[str] = None, timeout: int = 30):
        ...
        Args:
            api_url: Base URL for the Glean API (defaults to env variable or Glean prod endpoint)
            api_key: Optional API key for authentication (defaults to env variable)
            timeout: Request timeout in seconds
        """
        # Get from environment variables with proper defaults
        self.api_url = api_url or os.getenv("GLEAN_API_URL", "https://scio-prod-be.glean.com/rest/api/v1")
        self.api_key = api_key or os.getenv("GLEAN_API_KEY")
        self.agent_id = os.getenv("GLEAN_AGENT_ID", "1d14f9592fe34b288dec869a218d44de")
        
        if not self.api_key:
            raise ValueError("GLEAN_API_KEY environment variable is required. Please set it in your .env file.")
